...
| **Explainability (XAI)** | Black Box, or tabular-based SHAP analysis. | **1D Time-Series Integrated Gradients.** | Generates temporal "Suspicion Heatmaps" for forensic, operationally complete justification (legal defensibility requires regulatory and forensic validation outside the scope of this thesis). |
...
As demonstrated in the experimental results, standard industrial baselines trained without deep cascading architecture (such as standard edge XGBoost) achieve F1-scores of $0.854$, while standard academic baselines trained without physics-grounded Digital Twin augmentation and Context-Aware GLI features suffer from massive False Positive Fatigue (yielding a preliminary uncalibrated precision baseline of approximately $8\%$). By bridging these gaps, the GridGuard ensembled model achieves a mathematically locked F1-score of **$90.5\%$** ($91.1\%$ Precision, $89.8\%$ Recall).
...
In the utility sector, cutting a customer's power based on a "black box" algorithm is operationally high-risk. GridGuard AI pioneers the application of **Integrated Gradients** mapped specifically to 1D electricity consumption sequences. Rather than just giving a probability score, the system outputs a color-coded **Temporal Heatmap**, allowing human field technicians to see the exact day and hour the bypass was initiated. Note: Legal defensibility requires regulatory and forensic validation outside the scope of this thesis.
