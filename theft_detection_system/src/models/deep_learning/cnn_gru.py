...
cnn_gru.py
----------
CNN-GRU hybrid binary classifier for theft detection.
...
------------
  ...
  GRU temporal encoder:
      nn.GRU(cnn_channels[1] → gru_hidden, num_layers, dropout)
      Last hidden state → (B, gru_hidden)

  Optional BatchNorm1d on hidden state

  FC head:
      Linear(gru_hidden → 64) → ReLU → Dropout → Linear(64 → 1) → Sigmoid
  Output : (B, 1)
"""
...
class CNNGRUClassifier(nn.Module):
    """CNN-GRU hybrid binary classifier.

    A two-block CNN extracts local temporal features, then a GRU models
    longer-range sequential patterns across CNN output frames.
    ...
    use_batch_norm : bool
        Apply BatchNorm in CNN blocks and on GRU hidden state. Default ``True``.
    fc_hidden : int
        FC head hidden size. Default ``64``.

    Examples
    --------
    >>> model = CNNGRUClassifier(input_dim=32, seq_len=50)
    ...
    def __init__(
        ...
        self.bn_hidden: Optional[nn.BatchNorm1d] = (
            nn.BatchNorm1d(gru_hidden) if use_batch_norm else None
        )
        ...
        self._init_weights()
        logger.info(
            "CNNGRUClassifier initialised | input_dim=%d | seq_len=%d "
            "| cnn_channels=%s | gru_hidden=%d | params=%d",
            input_dim,
            seq_len,
            cnn_channels,
            gru_hidden,
    ...
    def _init_weights(self) -> None:
        """Kaiming for Conv; Xavier/orthogonal for GRU; Xavier for Linear."""
    ...
    def get_cnn_features(self, x: torch.Tensor) -> torch.Tensor:
        """Return CNN feature maps (before GRU).
